...
import { Button, Dialog, Header, Text } from "@revolt/ui";
    ...
    <Dialog {...props}>
      ...
      <Flex direction="column" gap="4" p="4" maxH="70vh" overflowY="auto">
        <Flex direction="column" gap="2">
          <Text size="lg" weight="bold">Overview</Text>
          <Text size="sm" colour="muted">ID: {message.id}</Text>
          <Text size="sm" colour="muted">Created: {message.createdAt?.toLocaleString()}</Text>
          <Text size="sm" colour="muted">Author: {message.username} ({message.authorId})</Text>
        </Flex>

        <Flex direction="column" gap="2">
          <Text size="lg" weight="bold">Raw Data</Text>
          <RenderCodeblock>
            {codeToDisplay}
          </RenderCodeblock>
        </Flex>
